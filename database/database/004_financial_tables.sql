-- AutoNidhi — Part 4: Financial Ledgers
-- CHANGES:
--   payment_in    : + CHECK (paid + remaining = total)
--   expense_ledger: + file_id (nullable), created_by, created_at
--   advances      : REPLACED party_type+party_id with dealer_id+broker_id proper FKs
...
CREATE TABLE payment_in (
  ...
  remarks          TEXT,
  CONSTRAINT chk_payment_in_amounts CHECK (
    paid_amount IS NULL OR remaining_amount IS NULL OR
    (paid_amount + remaining_amount = payment_amount)
  )
);
...
-- expense_ledger — ADDED file_id, created_by, created_at
...
-- advances — FIXED: proper dealer_id + broker_id FKs
CREATE TABLE advances (
  id               UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  dealer_id        UUID REFERENCES master_dealer(id),
  broker_id        UUID REFERENCES master_broker(id),
  CONSTRAINT chk_advances_one_party CHECK (
    (dealer_id IS NOT NULL AND broker_id IS NULL) OR
    (broker_id IS NOT NULL AND dealer_id IS NULL)
  ),
