-- AutoNidhi — Part 3: Core Entities & Sub-Modules
-- CHANGES:
--   customer      : + city, state, pincode, date_of_birth, aadhar_number,
--                     pan_number, customer_type, created_by, created_at
--   file_record   : - service_type VARCHAR  → + file_type enum, status enum
--                     + assigned_to, + updated_at
--   insurance_info: + insurance_type_id FK (was completely missing)
...
CREATE TABLE customer (
  ...
  created_by    UUID REFERENCES system_user(id),
...
CREATE TABLE file_record (
  ...
  assigned_to         UUID REFERENCES system_user(id),
...
-- insurance_info — ADDED insurance_type_id
CREATE TABLE insurance_info (
  id                        UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  file_id                   UUID NOT NULL UNIQUE REFERENCES file_record(id),
  insurance_company_id      UUID REFERENCES master_insurance_company(id),
  insurance_type_id         UUID REFERENCES master_insurance_type(id),
