  ...
  return (
    <>
      <PageHeader title="Files" subtitle="All loan & insurance files"
        actions={<Link to="/data-entry/files/new" className="btn btn-primary btn-sm">+ New file</Link>} />
      ...
      <DataTable rows={filtered} searchKeys={["id", "customer", "bank"]} columns={[
        { key: "id", label: "File #", render: (r) => <a className="auth-link" onClick={() => navigate(`/data-entry/files/${r.id}`)}>{r.id}</a> },
