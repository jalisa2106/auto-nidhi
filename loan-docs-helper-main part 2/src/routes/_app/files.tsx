...
function FilesPage() {
  ...
  return (
    <>
      <PageHeader title="Files" subtitle="All loan & insurance files"
      ...
      <DataTable rows={filtered} searchKeys={["id", "customer", "bank"]} columns={[
        { key: "id", label: "File #", render: (r) => <a className="auth-link" onClick={() => router.navigate({ to: "/files/$id", params: { id: r.id } })}>{r.id}</a> },
